...
@Component({
          ...
          <div class="chart-content">
            <div class="simple-chart">
              <div class="chart-bars">
                <div *ngFor="let item of salesChart.data; let i = index" class="chart-bar">
                  <div class="bar-value" [style.height.%]="(item / maxChartValue) * 100"></div>
                  <div class="bar-label">{{ salesChart.labels[i] }}</div>
                </div>
              </div>
            </div>
  ...
  styles: [`
    ...
    .product-item:hover, .pharmacy-item:hover {
      background: #e2e8f0;
...
export class ReportsComponent implements OnInit {
  ...
  async loadMetrics() {
    try {
      // Get real statistics
      const [users, products, pharmacies] = await Promise.all([
        this.adminService.getUsers().toPromise(),
        this.adminService.getProducts().toPromise(),
        this.adminService.getPharmacies().toPromise()
      ]);

      const totalRevenue = Math.floor(Math.random() * 50000) + 10000;
      const totalOrders = Math.floor(Math.random() * 1000) + 500;
      const productsSold = Math.floor(Math.random() * 5000) + 2000;
  ...
  async loadChartData() {
    const data = [1200, 1900, 800, 1500, 2000, 1700, 1400];
    this.maxChartValue = Math.max(...data);
  ...
  async loadTopProducts() {
    try {
      ...
      this.topProducts = [
        { name: 'Paracetamol 500mg', sales: 145, revenue: '2,890' },
        { name: 'Amoxicillin 250mg', sales: 132, revenue: '2,640' },
        { name: 'Ibuprofen 400mg', sales: 98, revenue: '1,960' },
        { name: 'Vitamin D3', sales: 87, revenue: '1,740' },
        { name: 'Aspirin 100mg', sales: 76, revenue: '1,520' }
      ];
    }
  }
